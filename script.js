...
const audio7 = document.querySelector(`audio[data-key="7"]`)
const audio8 = document.querySelector(`audio[data-key="8"]`)
...
const Keyboard = {
  ...
  init() {
    ...
    keys = this.language === 'en' ? keysEn : keysRu
  ...
  createRowButtons(arr) {
    ...
    arr.forEach(item => {
      ...
      if (Array.isArray(keys[item])) {
        ...
        if (keys[item].match(/Backspace|Tab|CapsLock|Enter|Shift| /)) {
  ...
  switch (code) {
    ...
    default :
      if (Keyboard.language === 'en') {
        new Audio('./assets/sound/sound-7.mp3').play()
        // audio7.play()
      } else {
        new Audio('./assets/sound/sound-8.mp3').play()
        // audio8.play()
      }
    break
  }
